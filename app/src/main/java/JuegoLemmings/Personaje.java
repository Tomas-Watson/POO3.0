...
public class Personaje extends ObjetoGrafico implements ObjetoGraficoMovible {
    public enum EstadoPersonaje {
        CAYENDO, CAMINANDO, EXCAVANDO, BLOQUEANDO, PLANEANDO, TREPANDO, MURIENDO, SALVADO, VELOZ, NUCLEAR
    }
    ...
    public void moverse(double delta) {
        ...
        // arrancamos a contar cuando entramos en CAYENDO
        if (estadoActual != EstadoPersonaje.CAYENDO
                && terreno.esSolido((int) positionX, (int) (positionY + getHeight_frame())) == false) {
        ...
        if (terreno.esSolido(footX, footY)) {
            // al aterrizar, medimos cuántos píxeles hemos caído
        ...
        if (estadoActual == EstadoPersonaje.CAMINANDO) {
            ...
            if (terreno.esSolido(sideX, sideY)) {
                if (basherActive) {
                    estadoActual = EstadoPersonaje.EXCAVANDO;
                ...
                if (puedeTrepar) {
                    ...
                    int height = 0;
                    // Contar bloques contiguos comenzando en 'row'
                    ...
                    // Comprobar que justo encima de la cima no haya bloque
                    if (!(topRow > 0 && terreno.mapNivel[col][topRow - 1] != 0)) {
                        // Calcular la Y final en píxeles para posicionar encima de la cima
                        treparHastaY = topRow * Terreno.baldosa - h;

                        // Ajustar estado y posición X lateral para empezar la animación de trepar
